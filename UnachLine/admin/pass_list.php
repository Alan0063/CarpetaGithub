...
<main>
    <?php if(isset($_SESSION['adminId'])) { ?>
      <div class="container-md mt-2">
        <h1 class="display-4 text-center text-secondary">Lista de Pasajeros</h1>
        <table class="table table-bordered table-sm">
          ...
          <tbody>
            ...
            $sql_t = 'SELECT * FROM ticket WHERE id_vuelo=?';
            $stmt_t = mysqli_stmt_init($conn);
            if(!mysqli_stmt_prepare($stmt_t,$sql_t)) {
                header('Location: ticket.php?error=sqlerror');
                exit();            
            } else {
                mysqli_stmt_bind_param($stmt_t,'i',$id_vuelo);            
                      ...
                      if ($row_p = mysqli_fetch_assoc($result_p)) {
                        ...
                        if ($row_u = mysqli_fetch_assoc($result_u)) {
                          ...
                          <tr class='text-center'>
                            ...
                            <td>".$row['telefono']."</td>
                            <td>".$row['dob']."</td>
                            <td scope='row'>".$row_u['nombre_usuario']."</td>
                            <td>$ ".$row_p['monto']."</td>
                          </tr>
                          "; 
                        }                       

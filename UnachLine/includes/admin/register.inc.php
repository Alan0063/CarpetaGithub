...
if(isset($_SESSION['adminId']) && isset($_POST['signup_submit'])) {    
    ...
    if(!filter_var($email_id,FILTER_VALIDATE_EMAIL)) {
        ...
        $username_sql = 'SELECT nombre_admin FROM Admin WHERE nombre_admin=?';
        $email_sql = 'SELECT email_admin FROM Admin WHERE email_admin=?';
        ...
        if($username_check > 0) {
            ...
            if($email_check > 0) {
                ...
                $sql = "INSERT INTO Admin(nombre_admin,email_admin,pwd_admin) VALUES
                  (?,?,?)";

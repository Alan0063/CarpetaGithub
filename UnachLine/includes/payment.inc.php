...
if(isset($_POST['pay_but']) && isset($_SESSION['userId'])) {
    ...
    $expiry = $_POST['cc-exp'];  
    $sql = 'INSERT INTO pago (id_usuario,expire_date,monto,id_vuelo,card_no) 
        VALUES (?,?,?,?,?)';            
            ...
            if(!mysqli_stmt_prepare($stmt,$sql)) {
                ...
                if($row = mysqli_fetch_assoc($result)) {
                    ...
                    if($class === 'B') {
                        ...
                        $seats = $seats - 1;
                        $stmt = mysqli_stmt_init($conn);
                        $sql = 'UPDATE vuelo SET last_seat=?, asientos=?
                            WHERE id_vuelo=?';
                        if(!mysqli_stmt_prepare($stmt,$sql)) {
                            header('Location: ../payment.php?error=sqlerror');
                            exit();            
                    ...
                    $stmt = mysqli_stmt_init($conn);
                    $sql = 'INSERT INTO Ticket (id_pasajero,id_vuelo
                        ,seat_no,cost,class,id_usuario
                        ) VALUES (?,?,?,?,?,?)';            
                    if(!mysqli_stmt_prepare($stmt,$sql)) {
                        header('Location: ../payment.php?error=sqlerror');
                        exit();            

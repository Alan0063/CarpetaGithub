        ...
        <?php if(isset($_POST['search_but'])) { 
          ...
          <div class="container-md mt-2">
            <h1 class="display-4 text-center "
              >Vuelos Disponibles: <br> <?php echo $dep_city; ?> 
                 hacia <?php echo $arr_city; ?> </h1>
            <table class="table table-striped table-bordered table-hover">
              ...
              <tbody>
                <?php
                $sql = 'SELECT * FROM vuelo WHERE salida=? AND Destino =? AND 
                    DATE(fecha_salida)=? ORDER BY Precio';
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                  $price = (int)$row['precio']*(int)$passengers;
                  if($type === 'round') {
                    $price = $price*2;
                  }
                  if($f_class == 'B') {
                  ...
                  if(isset($_SESSION['userId']) && $row['status'] === '') {   
                    ...
                    <input name='flight_id' type='hidden' value=".$row['id_vuelo'].">
